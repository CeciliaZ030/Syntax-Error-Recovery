...
We tested our program on cycle1 csug machine and the input file contained the example we used while testing. We also used the sample given on the assignment page, which is the case that calculates the first n primes, and the program was able to output the abstract syntax tree as expected. In order to demonstrate the structure of the abstract syntax tree, we implemented the indentation in the process of printing the abstract syntax tree which mirrors the internal structure of the tree. 
>>>>>>> db301d803bbcc2ccfe5e2abafb974c20f4681115
